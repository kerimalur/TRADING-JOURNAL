...
// Backtest-spezifischer Trade (NICHT im Hauptjournal)
...
interface BacktestSession {
  ...
  isPaused: boolean;
  elapsedMs: number;
  isCompleted?: boolean; // Abgebrochen/Beendet
}
...
export function Backtest() {
  ...
  // Keyboard Shortcuts - useCallback für stabile Referenz
  const handleKeyboardShortcut = useCallback((e: KeyboardEvent) => {
    ...
    switch(e.key.toLowerCase()) {
      ...
      case 's':
        ...
        break;
      case 'w':
        setFormData(prev => ({ ...prev, result: 'win', rMultiple: Math.abs(prev.rMultiple) }));
        break;
      case 'x':
        setFormData(prev => ({ ...prev, result: 'loss', rMultiple: -Math.abs(prev.rMultiple) }));
        break;
      case 'b':
        setFormData(prev => ({ ...prev, result: 'breakeven', rMultiple: 0 }));
        break;
    }
  }, []);
  ...
  const stats: BacktestStats = (() => {
    ...
    if (trades.length === 0) {
      return { totalTrades: 0, wins: 0, losses: 0, winRate: 0, totalR: 0, avgR: 0, profitFactor: 0 };
    }
  ...
  const handleSubmit = useCallback(() => {
    ...
    const updatedSession: BacktestSession = {
      ...currentSession,
      trades: [...currentSession.trades, newTrade],
      updatedAt: Date.now(),
    ...
    setFormData(prev => ({ 
      ...prev, 
      rMultiple: prev.result === 'win' ? 1 : prev.result === 'loss' ? -1 : 0,
      screenshot: ''
    }));
  ...
  const togglePause = () => {
    ...
    const updatedSession: BacktestSession = {
      ...
      elapsedMs: currentSession.isPaused 
        ? currentSession.elapsedMs 
        : currentSession.elapsedMs + (now - currentSession.updatedAt),
      updatedAt: now,
    };
  ...
  const createNewSession = () => {
    const newSession: BacktestSession = {
      ...
      name: `Backtest ${new Date().toLocaleString('de-DE', { 
        day: '2-digit', 
        month: '2-digit', 
        year: '2-digit',
        hour: '2-digit',
  ...
  const toggleSetup = (setupKey: string) => {
    setFormData(prev => ({
      ...prev,
      setups: prev.setups.includes(setupKey)
        ? prev.setups.filter(s => s !== setupKey)
        : [...prev.setups, setupKey]
    }));
  };
  ...
  const stopSession = () => {
    ...
    const updatedSession: BacktestSession = {
      ...
      isPaused: true,
      isCompleted: true,
      elapsedMs: currentSession.elapsedMs + (now - currentSession.updatedAt),
      updatedAt: now,
    };
                ...
                {sessions.length === 0 ? (
                  ...
                  sessions.map(session => (
                    <div
                      key={session.id}
                      className={clsx(
                        'flex items-center justify-between p-3 hover:bg-glass-white cursor-pointer',
                        session.id === currentSessionId && 'bg-glass-accent'
                      )}
                      onClick={() => {
                    ...
                    >
                      <div>
                        ...
                        <div className="text-xs text-text-muted">
                          {session.trades.length} Trades • {new Date(session.createdAt).toLocaleDateString('de-DE')}
                        </div>
                      </div>
                      <button
                        onClick={(e) => {
      ...
      {!currentSession ? (
        <div className="card text-center py-16">
          ...
          <p className="text-text-muted mb-4">
            Erstelle eine neue Backtest-Session um zu starten.
          </p>
        ...
        <>
          {/* Session Header */}
          <div className="card mb-6 bg-gradient-to-r from-pnl-positive/10 to-accent-blue/10 border-l-4 border-pnl-positive">
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                {!currentSession.isCompleted && (
                  <>
                    <button
                      ...
                      className={clsx(
                        'btn',
                        currentSession.isPaused ? 'btn-primary' : 'btn-secondary'
                      )}
                    >
                    ...
                    <button 
                      onClick={() => confirm('Session wirklich beenden? Du kannst danach keine neuen Trades mehr hinzufügen.') && stopSession()} 
                      className="btn btn-secondary text-pnl-negative hover:bg-pnl-negative/20"
                    >
                      <StopCircle size={16} />
                      Beenden
                    </button>
                ...
                {currentSession.isCompleted && (
                  <span className="px-3 py-2 bg-text-muted/20 rounded-lg text-text-muted text-sm">
                    Session beendet
                  </span>
                )}
          ...
          <div className="grid grid-cols-3 gap-6">
            ...
            <div className="col-span-2 card">
              <h3 className="text-lg font-semibold mb-4 flex items-center gap-2">
                ...
                Speed Entry
                <span className="text-xs text-text-muted ml-2">(L=Long, S=Short, W=Win, X=Loss, B=BE, Enter=Submit)</span>
              </h3>
                      ...
                      <button
                        ...
                        className={clsx(
                          ...
                          formData.result === result
                            ? result === 'win' ? 'bg-pnl-positive text-white'
                              : result === 'loss' ? 'bg-pnl-negative text-white'
                    ...
                    {Object.entries(SETUP_DEFINITIONS).map(([key, setup]) => (
                      <button
                        ...
                        className={clsx(
                          ...
                          formData.setups.includes(key)
                            ? 'text-white'
                            : 'bg-background-surface-hover text-text-muted hover:text-text-primary'
                        )}
                        style={{
                          backgroundColor: formData.setups.includes(key) ? setup.color : undefined,
                        }}
                      >
                        {setup.short}
                      </button>
              ...
              <button
                ...
                className={clsx(
                  ...
                  currentSession.isCompleted && 'opacity-50 cursor-not-allowed'
                )}
              >
                <Save size={16} />
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={equityCurve}>
                    ...
                    <Line
                      ...
                      stroke="#FFD700"
                      strokeWidth={2}
                      dot={{ fill: '#FFD700', strokeWidth: 0, r: 3 }}
                    />
                  </LineChart>
                </ResponsiveContainer>
              </div>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {[...currentSession.trades].reverse().slice(0, 10).map((trade, i) => (
                      <tr key={trade.id} className="border-b border-border/50 hover:bg-glass-white">
                        ...
                        <td className="py-2 px-3">
                          <span className={clsx(
                            ...
                            trade.direction === 'long' ? 'bg-pnl-positive/20 text-pnl-positive' : 'bg-pnl-negative/20 text-pnl-negative'
                          )}>
                        ...
                        <td className="py-2 px-3">
                          <span className={clsx(
                            ...
                            trade.result === 'loss' ? 'bg-pnl-negative/20 text-pnl-negative' :
                        ...
                        <td className="py-2 px-3 text-center">
                          {trade.screenshot ? (
                            <img 
                              ...
                              className="h-8 w-12 object-cover rounded cursor-pointer hover:opacity-80"
