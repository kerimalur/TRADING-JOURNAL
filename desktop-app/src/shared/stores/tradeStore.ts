...
import type { Trade, TradeFilters, AccountType, DashboardMetrics } from '@/shared/types';
import { calculateDashboardMetrics, createMemoizedCalculator } from '@/shared/utils/calculations';
  ...
  saveTrade: async (trade, screenshot) => {
    try {
      ...
      if (savedTrade && screenshot) {
        const { getApi } = await import('@/shared/services/webApi');
        const api = getApi();
